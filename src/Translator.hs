...
--type TreeTransaltor a = AbstractProgramTree -> (a, AbstractProgramTree)
...
translateExpression _ (A.SLit sl) (sp, fp) = let (sid, sp') = insertAndGetId sp sl in
                                                 Right (T.SLit sid, (sp', fp))
translateExpression _ (A.ILit a) s = Right (T.ILit a, s)
translateExpression _ (A.DLit a) s = Right (T.DLit a, s)
translateExpression fid (A.UnaryExpression op e) s = case translateExpression fid e s of
...
--abstractToTranslatable t = let (a, b) = runState abstractProgramTreeTranslator ([], [])
                           --in b
